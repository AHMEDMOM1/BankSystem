...
public:
  ...
  clsDate(string sDate) {
    ...
    _Day = static_cast<short>(stoi(vDate[0]));
    _Month = static_cast<short>(stoi(vDate[1]));
    _Year = static_cast<short>(stoi(vDate[2]));
  }
  ...
  // we will add time to date for register login in file
  ...
  static clsDate GetSystemDate() {
    ...
    short sysDay, sysMonth, sysYear;

    sysYear = static_cast<short>(now->tm_year + 1900);
    sysMonth = static_cast<short>(now->tm_mon + 1);
    sysDay = static_cast<short>(now->tm_mday);

    return clsDate(sysDay, sysMonth, sysYear);
  }
  ...
  int DaysUntilTheEndOfMonth() { return DaysUntilTheEndOfMonth(*this); }
  ...
  int DaysUntilTheEndOfYear() { return DaysUntilTheEndOfYear(*this); }
  ...
  static clsDate CalculateVacationReturnDate(clsDate DateFrom,
                                             short VacationDays) {
  ...
  static bool isDateBetween(const clsDate &golDate, const clsDate &from,
                            const clsDate &to) {

    const clsDate &earlier{
        CompareDates(from, to) == enDateCompare::Before ? from : to};
    const clsDate &later{
        CompareDates(from, to) == enDateCompare::Before ? to : from};

    enDateCompare cmp1{CompareDates(earlier, golDate)};
    enDateCompare cmp2{CompareDates(later, golDate)};

    bool fromBool{cmp1 == enDateCompare::Before ||
                  cmp1 == enDateCompare::Equal};
    bool toBool{cmp2 == enDateCompare::After || cmp2 == enDateCompare::Equal};

    return fromBool && toBool;
  }

  bool siDateBetween(const clsDate &from, const clsDate &to) {
    return isDateBetween(*this, from, to);
  }
